...
# Add a "Home" button in the sidebar for navigation
with st.sidebar:
    if st.button("🏠 Home"):
...
st.sidebar.title(get_text("workspace_title"))
st.sidebar.caption("파일을 업로드하면 자동으로 생성되는 결과물 저장 폴더입니다.")
run_base_dir = st.sidebar.text_input(
...
st.sidebar.markdown("---")
st.sidebar.subheader(get_text("status_title"))
if "last_audio_input_media" in st.session_state:
    st.sidebar.info(f"{get_text('input_label')}{Path(st.session_state['last_audio_input_media']).name}")
