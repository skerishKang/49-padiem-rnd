...
def render():
    with st.expander("XTTS 백업 합성", expanded=True):
        ...
        with col_input:
            default_xtts_input = st.session_state.get("xtts_input_json_path", "data/intermediates/text_process_result.json")
            xtts_input = handle_file_input(
                "XTTS 입력 JSON 경로",
                "xtts_input_json_path",
                default_xtts_input,
                "XTTS 입력 JSON 업로드",
                ["json"],
            )
            ...
            default_xtts_output = st.session_state.get("xtts_output_path", "data/intermediates/tts_backup_output.wav")
        ...
        if st.button("XTTS 합성 실행"):
            ...
            if result:
                ...
                st.session_state["lipsync_input_audio_path"] = xtts_output
                ...
                save_session_data({
                    ...
                    "lipsync_input_audio_path": xtts_output,
                })
