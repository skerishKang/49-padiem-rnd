...
# Unified UI도 동일하게 8049 포트의 백엔드로 요청을 보낸다.
API_BASE_URL = "http://localhost:8049"
    ...
    if async_mode:
        ...
        if response and "job_id" in response:
            ...
            while True:
                ...
                if state == "success":
                    ...
                    # pending / running 상태에서는 점진적으로 진행률만 올려주고 상태 텍스트를 갱신
        ...
        if response:
            if response.get("status") == "success":
                ...
                st.error(f"실패: {response.get('message')}")
        return None
