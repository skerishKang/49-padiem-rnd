...
- 글로벌 수요 지표: 2024년 전통 더빙/영화 더빙 시장은 약 **40.4억 달러** 규모이며 2033년 **75.5억 달러**로 성장(연평균 7.19%) 전망 (출처: GlobalGrowthInsights, Film Dubbing Market, https://www.globalgrowthinsights.com/market-reports/film-dubbing-market-102061). 더빙·자막 통합 시장에서는 OTT 파트너십 비중이 **29%**, 2024년 출시 신제품의 **34%** 가 실시간 자막·다국어 AI 기능을 포함 (출처: GlobalGrowthInsights, Dubbing & Subtitling Market, https://www.globalgrowthinsights.com/market-reports/dubbing-and-subtitling-market-117679).
- 글로벌 수요 지표(시각 자료):
  - 영화 더빙 시장 규모 및 CAGR 그래프  
    ![Film Dubbing Market](최종보고서_이미지/film_dubbing_market_kr_1766512534168.png)
  ...
  - 더빙·자막 시장 규모 및 CAGR 그래프  
    ![Dubbing & Subtitling Market](최종보고서_이미지/film_dubbing_market_bar_kr.png)
...
- 비용 부담(전통 방식): 방송/극장급 더빙은 **분당 $50~60**(일부 $75까지) 수준, 기본 품질도 **분당 $20~30**이 일반적 (출처: Checksub, Dubbing Price Guide, https://www.checksub.com/blog/dubbing-price-guide ; BunnyStudio Blog, How much does a dub cost, https://bunnystudio.com/blog/how-much-does-a-dub-cost-the-latest-pricing-formula/). (환율 1,450원 적용 시: 기본 품질 **29,000~43,500원/분**, 방송·극장급 **72,500~87,000원/분**, 고급 사례 **108,750원/분**). 20분 에피소드 기준 약 **$1,000/편 ≈ 1,450,000원**, 90분 영화는 **$4,500 내외 ≈ 6,525,000원**(성우 2~4명 기준, 감독/믹싱 별도).

  > 상기 단가는 온라인 가이드의 “분당 x 러닝타임” 단순 곱 기준으로, 성우 수·디렉팅/재녹음 횟수·스튜디오 등급·믹싱/마스터링·번역/자막/QC 포함 여부에 따라 총액은 더 높아지는 것이 일반적임(실무 견적에서는 90분 단가가 6백만 원 이상으로 상승)
- 기간 부담(전통 방식): 준비·번역 2~3일, 캐스팅/계약 2~5일, 녹음 1~3일, 편집·믹싱·QC 2~4일 등 **언어당 1~3주**가 소요되며, 다언어 병렬 처리가 어려울 경우 언어 수만큼 시간이 증가. 이 때문에 중소 제작사·개인 창작자는 비용·기간 측면에서 글로벌 진출 장벽이 높음.
...
- 하이브리드 모델 구조(내부 구성) 시각 자료  
  ![Hybrid Model](최종보고서_이미지/hybrid_system_kr_v.png)
...
- 파이프라인 흐름도(내부 모듈 기준)  
  ![Pipeline Flow](최종보고서_이미지/pipeline_flow.png)
...
- STT→정규화→번역→길이/타이밍 정합 흐름도  
  ![STT Translation Flow](최종보고서_이미지/stt_translation_flow.png)
...
- 립싱크 참고 시각 자료:
  - Wav2Lip 공식 리포지토리 개요(모델 구조/데모)  
    ![Wav2Lip GitHub](최종보고서_이미지/wav2lip_github.png)
    - 출처: https://github.com/Rudrabha/Wav2Lip
  - MuseTalk 공식 리포지토리 개요(고해상도/실시간 강조)  
    ![MuseTalk GitHub](최종보고서_이미지/musetalk_github.png)
    - 출처: https://github.com/TMElyralab/MuseTalk
  - Wav2Lip ↔ MuseTalk 비교 썸네일(화질/입모양 대비)  
    ![LipSync Compare](최종보고서_이미지/lipsync_compare.png)
...
- GPU 가속 참고 시각 자료:
  - TensorRT-LLM 가속 사례(엔비디아 공식 블로그)  
    ![TensorRT LLM](최종보고서_이미지/tensorrt_llm.png)
  - GPU 최적화 스택(캐싱/fp16/체크포인팅/변환 검토)  
    ![GPU Stack](최종보고서_이미지/gpu_stack.png)
...
- 정량 지표 요약 그래프  
  ![Metrics Graph](최종보고서_이미지/metrics_graph.png)
