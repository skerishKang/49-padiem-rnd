...
- 근거(내부): `pdf/ocr/본문1 (2)/과제접수용연구개발계획서(PART2(본문1))_page-0024.jpg`(기대효과/장기 파급효과)
...
- 평가방법/평가환경(계획서 요약): 연구개발계획서에서는 성능지표별로 측정 정의와 활용 데이터셋/평가환경을 명시하였으며, 본 보고서의 지표 관리 체계는 해당 정의를 기준으로 구성하였음.
  - WER(음성 인식 정확도): 한국정보통신기술협회(TTA) 인증평가 기준의 단어 오류율로, 인식 결과와 정답 텍스트 간의 차이를 단어 단위로 계측하는 방식으로 정의하였음. 평가 데이터셋은 LibriSpeech, TIMIT 등 음성 데이터셋 활용을 명시하였고, 다양한 노이즈 환경(실제 녹음 환경)에서의 성능 검증을 고려하도록 서술되어 있음.
  - BLEU(기계 번역 품질): 한국정보통신기술협회(TTA) 인증평가 기준의 BLEU로, 번역 결과와 참고 번역 간 n-gram 기반 유사도를 계측하는 방식으로 정의하였음. 평가 데이터셋으로는 한국어-영어 번역 데이터셋(예: WMT 등) 활용을 명시하였고, 다양한 난이도의 문장(일반 대화/전문 용어 등)을 포함한 평가를 고려하도록 서술되어 있음.
  - PSNR(영상 품질): Peak Signal-to-Noise Ratio 기반으로 이미지/비디오의 품질을 계측하여 립싱크 결과의 화질 저하 여부를 확인하는 방식으로 정의하였음. 합성 결과가 원본 대비 품질 저하 없이 자연스러운 결과를 유지하는지를 확인하는 관점으로 운영하도록 명시되어 있음.
  - FID(생성 품질): Frechet Inception Distance 기반으로 생성 이미지와 실제 이미지 분포 간 차이를 계측하여 현실성을 평가하는 방식으로 정의하였음. ImageNet, CelebA 등 이미지 데이터셋 활용 및 Inception 기반 특징 추출을 통한 평가를 고려하도록 서술되어 있음.
  - 데이터셋 규모/용량: 충분한 학습 데이터 확보 여부와 언어/화자별 데이터 균형 확보를 통해 과적합 가능성을 낮추는 방향으로 데이터셋 규모와 품질을 관리하도록 명시되어 있음.
  - 언어 다양성(다국어 지원): 지원 가능한 언어 수 및 주요 언어 지원 범위를 기준으로 다국어 지원 범위를 평가·확인하도록 명시되어 있음.
