...
| 프론트엔드 | UI(원스톱 실행/미리보기) | `frontend/app.py`, `frontend/steps/*` | Streamlit 기반 |
| (검토) 보컬 분리(Demucs) | STT 품질 향상용 전처리 | `Research_Note_Combined.md` 내 Demucs 채택/연동 기록 | 본 레포 통합 파이프라인에는 미포함 |
| (검토) VAD 기반 무음 필터 | 환각 억제/연산 절감 | `Research_Note_Combined.md` 내 `vad_filter=True` 검토 기록 | 본 레포 Whisper 모듈에는 미적용 |
...
- 보컬/반주 분리(Stem Separation) 기술 검토: Demucs 기반 보컬 분리 도입을 연구노트에서 채택·테스트하였으나, 본 레포지토리의 통합 파이프라인에는 별도 모듈로 포함되지 않았음(후속 통합 대상).
  - 근거: (내부) `Research_Note_Combined.md` 내 "Demucs 채택", "Demucs 연동 및 테스트" 기록 / (외부) Défossez et al., "Music Source Separation in the Waveform Domain" (arXiv:1911.13254), https://arxiv.org/abs/1911.13254, https://github.com/facebookresearch/demucs
- VAD(Voice Activity Detection) 기반 무음 구간 처리 기술 검토: 무음 구간에서의 STT 환각 억제를 위해 VAD 적용을 연구노트에서 검토하였으나, 본 레포지토리의 Whisper STT 모듈은 `openai/whisper` 기반으로 구현되어 VAD 필터 옵션은 기본 제공되지 않으며, 후속 개선 항목으로 관리하였음.
  - 근거: (내부) `Research_Note_Combined.md` 내 "faster-whisper 내장 VAD 옵션(`vad_filter=True`)" 검토 기록 / (외부) `faster-whisper` VAD(Silero) 문서: https://github.com/SYSTRAN/faster-whisper , Silero VAD: https://github.com/snakers4/silero-vad

#### 근거 발췌(연구노트)

다음은 전처리(Demucs) 및 VAD 적용에 대한 연구노트의 핵심 기록 발췌이며, 본 레포 통합 파이프라인에 직접 포함되지 않았음을 병기하였음.

> (Research_Note_Combined.md / 20241104_데이터수집전략.md) "**Demucs 채택.** `pip install demucs`로 설치 가능하고 API로 제어하기 쉬움."
>
> (Research_Note_Combined.md / 20241105_자체데이터녹음.md) "`modules/audio/separator.py` 작성. 입력 오디오를 받아 `vocals.wav`와 `no_vocals.wav`로 분리하는 함수 구현."
>
> (Research_Note_Combined.md / 20241108_오디오슬라이싱.md) "**결과**: 성공. Demucs로 분리된 깨끗한 음성을 사용하니 STT 정확도가 10월 테스트 대비 약 15% 향상됨."
>
> (Research_Note_Combined.md / 20241023_VALLEX테스트및규격확정.md) "`faster-whisper` 내장 VAD 옵션(`vad_filter=True`) 활성화. **결과**: 무음 구간에서의 환각 현상 현저히 감소. 정확도 향상 확인."
...
- 초기 성능 계측 기록(내부): 5초 clip 기준 평균 추론 지연시간 342ms, throughput 2.7 clips/s, GPU 메모리 사용량 약 38GB를 기록하였음. 해당 수치는 연구노트 기반 초기 기록이며, 시험성적서용 공식 수치로는 별도 재현 및 검증이 필요함.
  - 근거: `notev2/Kanghyerim/2024-10/20241016.md`
- 성능 리포트 자동화 도구(스켈레톤): clip 길이별 지연/메모리/throughput을 CSV로 기록하는 스크립트 골격을 `finalv2/scripts/benchmark_runner.py`로 구성하였음.
  - 근거: `finalv2/scripts/benchmark_runner.py`, `test_reports_company/docs/process/measurement_commands_v1.md`, `notev2/Kanghyerim/2024-10/20241016.md`
...
> (Research_Note_Combined.md / 20250120_화자분리데이터.md) "`cProfile` 및 `time` 모듈로 단계별 소요 시간 측정. **결과**: VALL-E X 생성이 전체 시간의 60%, RVC가 30%, 나머지가 10%. **병목**: VALL-E X의 AR 모델 추론이 가장 느림."
...
> (Research_Note_Combined.md / 20250409_모델서빙최적화.md) "VALL-E X와 RVC 모델 변환 시도. **결과**: 일부 커스텀 연산(Custom Ops) 지원 문제로 변환 실패. 난이도 높음. ... **결정**: 일단 Python 런타임 최적화(JIT Compile 등)에 집중하고 모델 변환은 보류." / "Skip TorchScript/ONNX for now. Focus on caching."
...
> (Research_Note_Combined.md / 20250120_화자분리데이터.md) "**한계**: Python 코드 레벨에서의 최적화는 한계가 있음. -> 추후 C++ 포팅이나 TensorRT 변환 고려 (먼 미래)."
...
- 백엔드/프론트엔드: FastAPI 기반 백엔드와 Streamlit 기반 UI를 구축하여, 입력 파일 지정 → 옵션 선택 → 실행 → 결과 확인의 원스톱 UX를 제공하였음.
  ...
  - 프론트엔드 근거: `frontend/app.py`, `frontend/steps/*`
...
- 품질 관리(정량 평가) 체계: 본 레포지토리의 서비스 파이프라인과 별도로, 정량 지표 산출은 평가 스크립트/결과 JSON 형태로 관리하였음.
  - WER 산출 스크립트: `finalv2/scripts/measure_wer.py`, `finalv2/scripts/measure_wer_normalized.py`
  - BLEU 산출 스크립트: `finalv2/scripts/measure_bleu.py`
  - PSNR 산출 스크립트: `finalv2/scripts/measure_psnr_fid.py` (1차 버전: PSNR만 계산)
  - 결과 예시(대표 샘플):
...
- 근거(내부 문서): `test_reports_company/reports/2025_phase3_system_v1_test_report_draft.md` 5.2절(IRIS 연구개발계획서 정량목표 요약 표)
...
- 내부 기록: VALL-E X 연동 테스트 과정에서 MOS 4.3 달성(기존 3.8 대비 개선) 기록이 확인되었음.
  - 근거: (내부) `Research_Note_Combined.md` 내 "VALL-E X 연동 테스트" 및 "MOS(Mean Opinion Score) 4.3 달성" 기록 / (내부) `Test_Report_limone_dev.pdf` p.2(OCR: "MOS (Mean Opinion Score) ... 4.38 4.47 4.00" / LibriSpeech Test-Other, single A100) / (외부) ITU-T P.800.2(Mean opinion score interpretation and reporting): https://www.itu.int/rec/T-REC-P.800.2-201607-I/en , ITU-T P.800.1(Mean Opinion Score(MOS) terminology): https://www.itu.int/rec/T-REC-P.800.1-200303-S/en
  - 근거 스크린샷(외부): `attachments/external/ext_mos_itu_p8002.png`, `attachments/external/ext_mos_itu_p8001.png`

- 근거 발췌(연구노트):

> (Research_Note_Combined.md / 20250416_HiFiGAN평가.md) "**결과**: 기존 Vocoder 대비 기계음이 현저히 감소. 고음역대(High Frequency)가 선명해짐. MOS(Mean Opinion Score) 4.3 달성 (기존 3.8)."
...
- 대표 샘플 계측(초기): 한국어(민형배) 샘플에서 normalized WER가 Whisper medium 17.143%, Whisper large-v3 10.476%로 계측되었음.
 ...
 - 산출 방식(스크립트): JSONL 페어(`ref`, `hyp`)를 입력으로 하여 공백 기반 토큰화 후 Levenshtein 거리로 WER(%)를 계산하고, 참조 토큰 수 기준 가중 평균을 산출하였음.
  - 근거: (내부) `finalv2/scripts/measure_wer.py` / (내부) `Test_Report_limone_dev.pdf` p.2(OCR: "WER (Word Error Rate)", "Best Performance: 3.69 WER", LibriSpeech Test-Other, single A100) / (외부) NIST SCTK sclite(WER 정의): https://github.com/usnistgov/SCTK/blob/master/doc/sclite.htm
  - 근거 스크린샷(외부): `attachments/external/ext_wer_nist_sclite.png`
...
- 대표 샘플 계측(초기): 한국어(민형배) 샘플 기준 BLEU가 Whisper medium 44.23, Whisper large-v3 41.50으로 계측되어 목표치(41.0) 이상을 달성하였음.
 ...
 - 산출 방식(스크립트): 공백 기반 토큰화 후 1~4-gram corpus BLEU를 0~100 스케일로 산출하도록 구현하였음.
  - 근거: (내부) `finalv2/scripts/measure_bleu.py` / (내부) `Test_Report_limone_dev.pdf` p.3(OCR: "Machine Translation Quality (BLEU)" 및 언어쌍별 BLEU 표: En-Ko 43.87 등) / (외부) Papineni et al., 2002, "Bleu: a Method for Automatic Evaluation of Machine Translation": https://aclanthology.org/P02-1040/
  - 근거 스크린샷(외부): `attachments/external/ext_bleu_acl_p02_1040.png`
...
- 대표 샘플 계측(초기): Wav2Lip 통합 시뮬레이션 영상 5건에서 ref/gen 페어가 동일하여 PSNR이 Infinity로 계측되었음(MSE=0). 이는 파이프라인 통합이 프레임 손실 없이 동작함을 확인하는 스모크 테스트 성격임.
 ...
 - 산출 방식(스크립트): OpenCV로 두 영상의 프레임을 순차 로딩하여 MSE를 계산하고 PSNR(dB)을 산출하도록 구현하였음.
  - 근거: (내부) `finalv2/scripts/measure_psnr_fid.py` / (외부) MathWorks PSNR 함수 문서(PSNR/MSE 정의 및 peak value 설명): https://www.mathworks.com/help/images/ref/psnr.html , NI "Peak Signal-to-Noise Ratio as an Image Quality Metric"(PSNR/MSE 설명): https://www.ni.com/en/shop/data-acquisition-and-control/add-ons-for-data-acquisition-and-control/what-is-vision-development-module/peak-signal-to-noise-ratio-as-an-image-quality-metric.html , OpenCV 공식 튜토리얼(PSNR 정의 및 MSE 기반 계산): https://docs.opencv.org/4.x/d5/dc4/tutorial_video_input_psnr_ssim.html
  - 근거 스크린샷(외부): `attachments/external/ext_psnr_mathworks.png`, `attachments/external/ext_psnr_ni.png`, `attachments/external/ext_psnr_opencv.png`
 - FID: 본 과제 범위의 코드에서는 `fid_score: None`으로 placeholder 상태이며, Inception 기반 특징 추출 파이프라인은 후속 버전에서 구현·측정할 계획임.
  - 근거: (내부) `Test_Report_limone_dev.pdf` p.4(OCR: "Video Lip-Sync Quality Comparison: Wav2Lip-HQ vs. MuseTalk", "FID Score (Lower is Better)" Wav2Lip-HQ 23.18 / MuseTalk 6.43) / (외부) Heusel et al., 2017, "GANs Trained by a Two Time-Scale Update Rule Converge to a Local Nash Equilibrium" (arXiv:1706.08500), https://arxiv.org/abs/1706.08500
  - 근거 스크린샷(외부): `attachments/external/ext_fid_arxiv_1706_08500.png`
...
- **[현 상태]** `finalv2/scripts/measure_psnr_fid.py`는 `fid_score: None`으로 placeholder 처리되어 있으며, FID 자동 측정은 미구현 상태임.
...
- 근거(내부): `Test_Report_limone_dev.pdf` p.5(OCR: "Conclusion All performance metrics exceed the target goals ... achieve the required standards for deployment")
...
음성 추출–STT–번역–TTS–음색 변환–립싱크로 이어지는 전체 과정을 모듈화하고, orchestrator 기반 파이프라인 제어 및 비동기 작업(Job) 실행 구조를 통해 재현 가능한 End-to-End 실행 경로를 확보하였음.
  - 근거: `orchestrator/pipeline_runner.py`, `orchestrator/config.yaml`, `backend/main.py`, `backend/job_manager.py`, `backend/routers/*`, `frontend/app.py`, `frontend/steps/*`
