...
- 글로벌 수요 지표: 2024년 전통 더빙/영화 더빙 시장은 약 **40.4억 달러** 규모이며 2033년 **75.5억 달러**로 성장(연평균 7.19%) 전망 [[GlobalGrowthInsights, Film Dubbing Market](https://www.globalgrowthinsights.com/market-reports/film-dubbing-market-102061)]. 더빙·자막 통합 시장에서는 OTT 파트너십 비중이 **29%**, 2024년 출시 신제품의 **34%** 가 실시간 자막·다국어 AI 기능을 포함 [[GlobalGrowthInsights, Dubbing & Subtitling Market](https://www.globalgrowthinsights.com/market-reports/dubbing-and-subtitling-market-117679)].
- 글로벌 수요 지표(시각 자료):
  - 영화 더빙 시장 규모 및 CAGR 그래프  
    ![Film Dubbing Market](final_report_images/film_dubbing_market.png)
    - 출처: GlobalGrowthInsights (2024)
  - 더빙·자막 시장 규모 및 CAGR 그래프  
    ![Dubbing & Subtitling Market](final_report_images/dubbing_subtitling_market.png)
    - 출처: GlobalGrowthInsights (2024)
...
- 비용 부담(전통 방식): 방송/극장급 더빙은 **분당 $50~60**(일부 $75까지) 수준, 기본 품질도 **분당 $20~30**이 일반적 [[Checksub, Dubbing Price Guide](https://www.checksub.com/blog/dubbing-price-guide); [BunnyStudio Blog](https://bunnystudio.com/blog/how-much-does-a-dub-cost-the-latest-pricing-formula/)]. (환율 1,450원 적용 시: 기본 품질 **29,000~43,500원/분**, 방송·극장급 **72,500~87,000원/분**, 고급 사례 **108,750원/분**). 20분 에피소드 기준 약 **$1,000/편 ≈ 1,450,000원**, 90분 영화는 **$4,500 내외 ≈ 6,525,000원**(성우 2~4명 기준, 감독/믹싱 별도).
...
- 하이브리드 모델 구조(내부 구성) 시각 자료  
  ![Hybrid Model](final_report_images/hybrid_model.png)
...
| (검토) 보컬 분리(Demucs) | STT 품질 향상용 전처리 | `notev2/Kangchulwonv2/2024-11/20241104_데이터수집전략.md`, `notev2/Kangchulwonv2/2024-11/20241105_자체데이터녹음.md`, `notev2/Kangchulwonv2/2024-11/20241108_오디오슬라이싱.md` | 본 레포지토리 통합 파이프라인에는 미포함 |
| (검토) VAD 기반 무음 필터 | 환각 억제/연산 절감 | `notev2/Kangchulwonv2/2024-10/20241023_VALLEX테스트및규격확정.md` | 본 레포지토리 Whisper 모듈에는 미적용 |
...
- 파이프라인 흐름도(내부 모듈 기준)  
  ![Pipeline Flow](final_report_images/pipeline_flow.png)
...
> (notev2/Kangchulwonv2/2024-10/20241010_시스템전체구조정의.md) "- **목표**: 유튜브 등 영상 콘텐츠의 다국어 더빙을 자동화하는 시스템 개발."
>
> (notev2/Kangchulwonv2/2024-10/20241010_시스템전체구조정의.md) "- **입력**: MP4 비디오 파일."
>
> (notev2/Kangchulwonv2/2024-10/20241010_시스템전체구조정의.md) "- **출력**: 더빙된 MP4 비디오 파일."
> (notev2/Kangchulwonv2/2024-10/20241010_시스템전체구조정의.md) "- **입력**: MP4 비디오 파일." / "- **출력**: 더빙된 MP4 비디오 파일."
>
> (notev2/Kangchulwonv2/2024-10/20241010_시스템전체구조정의.md) "- **AI Models**: Whisper (STT), VALL-E X (TTS), RVC (VC), Wav2Lip (LipSync)."
>
> (notev2/Kangchulwonv2/2024-10/20241011_자막입력및AI모듈설계.md) "- **모듈형 구조 (Modular Architecture)** 채택." / "- 각 기능(STT, TTS, VC 등)을 독립적인 모듈로 개발하여 유지보수성 향상."
>
> (notev2/Kangchulwonv2/2024-10/20241011_자막입력및AI모듈설계.md) "- `Input Video` -> `Audio Extraction` -> `STT` -> `Translation` -> `TTS` -> `RVC` -> `Wav2Lip` -> `Output Video`."
>
> (notev2/Kangchulwonv2/2024-10/20241017_RVC및VALLEX분석.md) "- **Hybrid Approach**: VALL-E X로 1차 생성(발음, 언어 처리) -> RVC로 2차 보정(음색 강화)."
>
> (notev2/Kangchulwonv2/2024-10/20241018_Wav2Lip분석및주간정리.md) "- **STT**: Faster-Whisper (Large-v3)" / "- **TTS**: VALL-E X" / "- **VC**: RVC v2" / "- **LipSync**: Wav2Lip (+ GAN)"
>
> (notev2/Kangchulwonv2/2024-10/20241031_10월성과정리.md) "- **주요 산출물**: 요구사항 정의서, 시스템 아키텍처 설계도, 오디오 추출 및 STT 프로토타입 코드."
...
- STT→정규화→번역→길이/타이밍 정합 흐름도  
  ![STT Translation Flow](final_report_images/stt_translation_flow.png)
...
- TTS 참고 시각 자료:
  - VALL-E X(MS Research) 프로젝트 페이지(품질/유사도 강조)  
    ![VALL-E X](final_report_images/valle_x_ms.png)
...
- 립싱크 참고 시각 자료:
  - Wav2Lip 공식 리포지토리 개요(모델 구조/데모)  
    ![Wav2Lip GitHub](final_report_images/wav2lip_github.png)
    - 출처: https://github.com/Rudrabha/Wav2Lip
  - MuseTalk 공식 리포지토리 개요(고해상도/실시간 강조)  
    ![MuseTalk GitHub](final_report_images/musetalk_github.png)
    - 출처: https://github.com/TMElyralab/MuseTalk
  - Wav2Lip ↔ MuseTalk 비교 썸네일(화질/입모양 대비)  
    ![LipSync Compare](final_report_images/lipsync_compare.png)
...
- GPU 가속 참고 시각 자료:
  - TensorRT-LLM 가속 사례(엔비디아 공식 블로그)  
    ![TensorRT LLM](final_report_images/tensorrt_llm.png)
  - GPU 최적화 스택(캐싱/fp16/체크포인팅/변환 검토)  
    ![GPU Stack](final_report_images/gpu_stack.png)
...
### 3-1-1. One-Stop AI 더빙 파이프라인 원스톱 자동화 구현
...
- 자체 Orchestrator를 통해 단일 실행 요청만으로 전 과정을 자동 수행하도록 구성하여, 원스톱 실행 경로를 구축하였음.
...
- 정량 지표 요약 그래프  
  ![Metrics Graph](final_report_images/metrics_graph.png)
...
- WER 정의 참고(위키피디아)  
  ![WER Wikipedia](final_report_images/wer_wikipedia.png)
...
- BLEU 정의 참고(위키피디아)  
  ![BLEU Wikipedia](final_report_images/bleu_wikipedia.png)
...
- PSNR/FID 정의 인포그래픽  
  ![PSNR FID Infographic](final_report_images/psnr_fid_infographic.png)
...
- 릴리즈 게이트 흐름도  
  ![Release Gate](final_report_images/release_gate.png)
...
- 단계별 확산/로드맵 시각 자료  
  ![Roadmap](final_report_images/roadmap.png)
